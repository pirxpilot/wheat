...
var Renderers = module.exports = {
  ...
  sitemap: Git.safe(function feed(version, callback) {
    Step(
      function loadData() {
  ...
  page: Git.safe(function page(version, name, callback) {
    var page;
    Step(
      function loadData() {
        Git.getHead(this);
      ...
      function (err, head) {
        if (err) { callback(err); return; }
        Git.readFile(head, "description.markdown", Tools.fixEncoding(this));
      },
