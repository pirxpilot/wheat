...
var Helpers = {
  ...
  intro: function intro(markdown) {
    var intro = markdown.substr(0, markdown.indexOf("\n\n"));
    return Markdown.encode(intro);
  },
  thumbnail: function thumbnail(article) {
    var regex = /<img src="(htt\S+)"/ig,
      match = regex.exec(article.markdown);
    if(match) {
      // replace will only work on images served by picasaweb
      return match[1].replace("/s288/", "/s72/");
    } else {
      return null;
    }
  },
  ...
  formatUrl: function formatUrl(article, prefix)
  {
    var value, day, month, date;

    date = new Date(article.date);
    value = prefix || "";
    value += datetime.format(date, "/%Y/%I/");
    value += article.name + ".html";
    return value;
  }
};
